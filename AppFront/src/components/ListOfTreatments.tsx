...
const INITIAL_VISIBLE_COLUMNS = ["general", "status", "actions"];

export default function ListOfTreatments() {
    //@ts-ignore
    const navigate = useNavigate();
    const { auth } = useAuthStore();
    const { data: citas, isLoading: isLoadingCitas } = useFetchData(`/tratamientos/paciente/${auth?.user._id}`, null);
    ...
    const filteredItems = React.useMemo(() => {
        ...
        if (hasSearchFilter) {
            filteredCitas = filteredCitas.filter((cita) =>
                cita.general.toLowerCase().includes(filterValue.toLowerCase()),
            );
        }
        //@ts-ignore
        ...
        return (
            <div className="flex flex-col gap-4">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-default-400 text-small">Total {citas.length} tratamientos</span>
        ...
        return (
            <div className="py-2 px-2 flex justify-between items-center">
                <span className="w-[30%] text-small text-default-400">
                    {selectedKeys === "all"
                        ? "Todos los tratamientos seleccionadas"
    ...
    return (
        ...
        >
            ...
            <TableBody emptyContent={"Tratamientos no encontrados"} items={sortedItems}>
                {(item) => (
