...
const INITIAL_VISIBLE_COLUMNS = ["paciente", "motivo", "fecha", "status", "actions"];

export default function CitasforAdmin() {
    ...
    const pacientesConCitas = React.useMemo(() => {
        if (isLoadingPacientes || isLoadingCitas) return [];
        return pacientes.filter((paciente: any) => citas.some((cita: any) => cita.paciente === paciente._id));
    }, [pacientes, citas]);
    ...
    const filteredItems = React.useMemo(() => {
        ...
        if (hasSearchFilter) {
            filteredCitas = filteredCitas.filter((cita) => {
                const paciente = pacientesConCitas.find((paciente: any) => paciente._id === cita.paciente);
                return paciente.nombre.toLowerCase().includes(filterValue.toLowerCase());
            });
        }
        //@ts-ignore
    ...
    }, [citas, filterValue, statusFilter, pacientesConCitas]);
    ...
    }, [pacientesConCitas, isLoadingPacientes]);
